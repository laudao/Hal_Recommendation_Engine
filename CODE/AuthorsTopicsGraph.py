...
#		self.topicsGraph = None
	...
	def ldamodel_list(self, x):
		...
		ldamodellist = []
		for i in range(l):
			ldamodellist.append(models.LdaModel.load('lda/lda.model'+str(i)))
	...
	def getMostRelevantTopic(self, graph, text):
		...
		for ldamodel in self.ldamodel_list:
			print("i = " + str(i))
			with open('topics/start_id.txt', 'r') as f:
				start_id_list = f.read().splitlines()
			start_id = int(start_id_list[i])
			i+=1
			updated = 0 
			...
			if updated:
				""" get topic words in order to have the right topic id """
				for topic in topics_list:
					if max_topic == topic[0]:
						...
						if t is None:
							...
							for index in range(len(merging_info)):
								merging_info[index] = int(merging_info[index])
