...
class TopicCreation:
	def __init__(self, files, nb_passes, nb_topics, nb_words):
		...
		if not(os.path.isfile("topics_info.txt")):
			f = open("topics_info.txt", 'w')
			f.write("nb_topics: " + str(self.nb_topics) + "\n")
			f.write("nb_words: " + str(self.nb_words) + "\n")
			for filename in self.files:
				f.write(filename + "\n")
			f.close()
			print("Created file topics_info.txt containing num of topics per corpus, number of wordsper topic and filenames used to create the topics")
		else:
			f = open("topics_info.txt", 'a')
	...
	def nb_topics(self, x):
		...
		if os.path.isfile("topics_info.txt"):
	...
	def create():
		...
		if delete:
			...
			if os.path.isfile("topics_info.txt"):
				os.remove(os.path.join(cwd, "topics_info.txt"))
			if os.path.isfile("merging_info.txt"):
				os.remove(os.path.join(cwd, "merging_info.txt"))
		elif not(os.path.isfile('topic_num.txt')):
			TopicCreation.initialize_topic_number()
		...
		while filename != "-1":
			...
			while len(filename) == 0 or not(os.path.isfile(filename+".txt")) or (filename+".txt") in files:
				if filename == "-1":
					break
				elif not(os.path.isfile(filename+".txt")):
		...
		if files == []:
			...
			if os.path.isfile("topics_info.txt"):
				for filename in files:
					if filename in open("topics_info.txt").read():
