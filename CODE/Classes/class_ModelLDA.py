...
class ModelLDA:
    ...
    def clean_text_fr(doc, punctuation, lemma):
        ...
        parasit_word = "plus cette cet cela les d'un afin ainsi ils avon d'un the and".split()
        for w in parasit_word:
            words_stop_fr.add(w)
        punc_free = ''
        for ch in doc:
            if ch not in punctuation:
                ...
                punc_free += ' '
        normalized = " ".join(
            lemma.lemmatize(word) for word in punc_free.split())  # associ un lemne a un mot (dogs -> dog)
        stop_free = " ".join(
            [i for i in normalized.lower().split() if i not in words_stop_fr])  # on enlève les mots inutile
        res = []
